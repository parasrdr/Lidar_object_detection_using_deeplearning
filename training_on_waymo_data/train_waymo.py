...
Description: configured original script for training on waymo data sequences
...
from data_process.waymo_dataloader import create_train_dataloader, create_val_dataloader
...
from config.train_waymo_config import parse_train_configs
...
def main_worker(gpu_idx, configs):
    ...
    if False:
        model.load_state_dict(torch.load(None, map_location='cpu'))
        if logger is not None:
            logger.info('loaded pretrained model at "SFA3D\sfa\pretrained_models\model\epoch_300.pth' )
    ...
    if logger is not None:
        logger.info('Size of training set: {}'.format(len(train_dataloader) * configs.batch_size))
        logger.info('number of batches in training set: {}'.format(len(train_dataloader)))
    
    
    
    if logger is not None:
        logger.info('Size of validation set: {}'.format(len(val_dataloader) * configs.batch_size))
    ...
    for epoch in range(configs.start_epoch, configs.num_epochs + 1):
        ...
        if configs.distributed:
            ...
            # train for one epoch
        train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer,val_dataloader)
...
def train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer,val_dataloader):
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_dataloader)):
        ...
        losses.update(to_python_float(reduced_loss), configs.batch_size)
        ...
        if tb_writer is not None:
            if (global_step % configs.tensorboard_freq) == 0:
                ...
                tb_writer.add_scalars('Training', loss_stats, global_step)
            # Log message
        if logger is not None:
        ...
        val_loss = validate(val_dataloader, model, configs,tb_writer,logger,global_step)
        print(f'validation loss {val_loss}')

def validate(val_dataloader, model, configs, tb_writer,logger,global_step):
    losses = AverageMeter('Validation Loss', ':.4e')
    criterion = Compute_Loss(device=configs.device)
    # switch to train mode
    ...
    with torch.no_grad():
        ...
        if tb_writer is not None:
            if (global_step % configs.tensorboard_freq) == 0:
                loss_stats['avg_loss'] = losses.avg
                loss_stats['total_loss']= validation_total_loss/batch_size
                tb_writer.add_scalars('Validation', loss_stats, global_step)
