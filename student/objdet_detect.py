...
def load_configs_model(model_name='darknet', configs=None):
    ...
    elif model_name == 'fpn_resnet_waymo':
        ####### ID_S3_EX1-3 START #######     
        #######
        configs.model_path = os.path.join(parent_path, 'tools', 'objdet_models', 'resnet')
        configs.pretrained_filename = os.path.join(configs.model_path, 'pretrained', 'fpn_resnet18_trained_on_waymo_dataset_epoch_10.pth')
        configs.arch = 'fpn_resnet_waymo'
        configs.pin_memory = True
...
def create_model(configs):
    ...
    elif (configs.arch == 'fpn_resnet_waymo'):
        print('using FPN-ResNet trained on waymo dataset 10 epochs')
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        elif 'fpn_resnet' or 'fpn_resenet_waymo' in configs.arch:
    ...
    for object in detections:
        class_id, bev_x, bev_y, z, h, bev_w, bev_l, yaw = object
        
        ## perform the conversion from BEV map coordinates to metric world cornidates using the limits for x, y and z set in the configs structure
        ...
        ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure...only keep objects that lie within the region of interest.
